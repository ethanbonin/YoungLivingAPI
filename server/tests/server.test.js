...
describe('POST /yl/login', () => {
    it('should return a response from good login', function (done) {
      ...
      .expect((res) => { // This creates a custom Expect
        ...
        expect(body).toExist();
        expect(body).toIncludeKeys(['userId',
                                    'userName',
                                    'displayName',
                                    'memberId',
                                    'signature'
...
describe('POST /yl/report_data', () => {
    var authtoken = '';
    it('should return a json object full of user data and the keys should equal the values in order for the first index of the array', function (done) {
      request(app)
      .post('/v0/yl/login')
      .send(good_login_body)
      .expect(200)
...
describe('POST /yl/about_to_go_inactive', () => {
  var authtoken = '';
  it('should return users who are about to go inactive and the keys should equal the values in order for the first index of the array', function (done) {
    request(app)
    .post('/v0/yl/login')
    .send(good_login_body)
    .expect(200)
